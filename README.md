...
Machine Learning algorithms are often taught through static equations and black-box libraries. This project aims to **demystify** these algorithms by making them visible. We believe that **seeing** how centroids move in K-Means or how itemsets grow in FP-Growth builds a deeper intuition than code alone.

## 🏗 System Architecture

The application follows a clean **Frontend-Compute-State** flow:

```mermaid
graph LR
    A[Frontend UI] -->|User Input| B(Compute Engine)
    B -->|Algorithm Steps| C{State Management}
    C -->|Updates| A
```

- **Frontend**: React + Vite for a high-performance interactive UI.
- **Compute**: Client-side execution of ML algorithms (no backend latency).
- **State**: React Context API for managing algorithm steps and global themes.
